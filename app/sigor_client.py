...
class SigorClient:
    ...
    async def authenticate(self, cpf: str, senha: str, unidade: int) -> str:
        ...
        IMPORTANT: endpoint is /gettoken (lowercase!). /getToken returns 401.
        """
        ...
        if not token or not token.startswith("Bearer "):
            msg = data.get("mensagem", "Token nao retornado")
            raise SigorAPIError(f"Auth failed: {msg}")

        self.token = token
        self.token_expiry = time.time() + 3500
        logger.info("SIGOR auth successful, token obtained")
        return token
    ...
    async def _ensure_token(self):
        ...
        if not self._credentials:
            raise SigorAPIError("No credentials stored. Call authenticate() first.")
        logger.info("Token expired or missing, re-authenticating...")
        await self.authenticate(
            cpf=self._credentials["cpfCnpj"],
            senha=self._credentials["senha"],
            unidade=self._credentials["unidade"],
        )

    def _auth_headers(self) -> dict:
        return {"Authorization": self.token}

    # === Reference Lists (APIs 2-6) - ALL USE GET, NOT POST! ===

    async def get_classes(self) -> list:
        await self._ensure_token()
        resp = await self._http.get(f"{self.base_url}/retornaListaClasse", headers=self._auth_headers())
        return self._parse_list_response(resp, "classes")

    async def get_unidades(self) -> list:
        await self._ensure_token()
        resp = await self._http.get(f"{self.base_url}/retornaListaUnidade", headers=self._auth_headers())
        return self._parse_list_response(resp, "unidades")

    async def get_tratamentos(self) -> list:
        await self._ensure_token()
        resp = await self._http.get(f"{self.base_url}/retornaListaTratamento", headers=self._auth_headers())
        return self._parse_list_response(resp, "tratamentos")

    async def get_estados_fisicos(self) -> list:
        await self._ensure_token()
        resp = await self._http.get(f"{self.base_url}/retornaListaTipoEstado", headers=self._auth_headers())
        return self._parse_list_response(resp, "estados_fisicos")

    async def get_acondicionamentos(self) -> list:
        await self._ensure_token()
        resp = await self._http.get(f"{self.base_url}/retornaListaAcondicionamento", headers=self._auth_headers())
        return self._parse_list_response(resp, "acondicionamentos")
    ...
    async def get_manifesto(self, mtr_numero: str) -> dict:
        """API 13: GET /retornaManifesto/{manNumero}"""
        await self._ensure_token()
        resp = await self._http.get(
            f"{self.base_url}/retornaManifesto/{mtr_numero}",
        ...
        if not result:
            msg = data.get("mensagem", "MTR nao encontrado")
            raise SigorAPIError(f"MTR {mtr_numero}: {msg}")
        return result

    # === MTR Receive in Batch (API 14) ===

    async def receber_manifesto_lote(self, payload: list) -> dict:
        """API 14: POST /receberManifestoLote"""
        await self._ensure_token()
        resp = await self._http.post(
            f"{self.base_url}/receberManifestoLote",
            json=payload,
            headers={**self._auth_headers(), "Content-Type": "application/json"},
        )
        if resp.status_code != 200:
            raise SigorAPIError(
                f"receberManifestoLote failed: HTTP {resp.status_code} - {resp.text[:200]}",
                resp.status_code,
            )
        data = resp.json()
        logger.info(f"receberManifestoLote response: {data.get('mensagem')}")
        return data

    # === Helper: Build receive payload from API 13 data ===
    ...
    def build_receive_payload(self, manifesto, responsavel, data_recebimento_ms,
        ...
        """Build a single item for receberManifestoLote from API 13 response."""
        residuos_payload = []
        for res in manifesto.get("listaManifestoResiduo", []):
            ...
            item = {
                ...
                "marQuantidade": quantidade,
                "marQuantidadeRecebida": qtd_recebida,
                "traCodigo": res["tratamento"]["traCodigo"],
                "uniCodigo": res["unidade"]["uniCodigo"],
                "tieCodigo": res["tipoEstado"]["tieCodigo"],
                "tiaCodigo": res["tipoAcondicionamento"]["tiaCodigo"],
                "claCodigo": res["classe"]["claCodigo"],
            }
            if qtd_recebida != quantidade:
                item["marJustificativa"] = justificativa or f"Quantidade recebida diferente: {qtd_recebida} vs {quantidade}"
            residuos_payload.append(item)

        placa = (placa_override or manifesto.get("manPlacaVeiculo", "")).replace("-", "")[:7]

        return {
            "manNumero": manifesto["manNumero"],
            "dataRecebimento": data_recebimento_ms,
            "nomeMotorista": motorista_override or manifesto.get("manNomeMotorista", ""),
            "placaVeiculo": placa,
            "nomeResponsavelRecebimento": responsavel,
